 ...
 */
!function (inCJS, inAMD, Array, Object, RegExp, scope, undefined) {
  ...
  function initPanzer(require) {
    var
      ...
      // get genData, based on environment
      ...
      testNodeKey = function (key, name, value, panzer) {
        var
        ...
        return panzer.d.some(function (pkgDef) {
      ...
      genNodes = genData.spawn(
        function (name, value, parent, flags) {
          var
            ...
            isBadKey = parent && testNodeKey('n', name, value, panzer),
            // flag whether this item is a tag key, only if this is not the first node
            isTagKey = parent && !isBadKey && testNodeKey('a', name, value, panzer)
          ;
          ...
          if (isBadKey || isTagKey) {
            ...
            node.index = flags.returns.length + 1;
            // capture depth
            ...
            if (parent) {
              ...
              if (node.childIndex) {
                ...
                flags.returns[node.previousIndex - 1].nextIndex = node.index;
              }
            }
            // if not the first node and there are packages...
            if (parent && panzer.d) {
              // step through packages of the current panzer backwards (most recent package gets precedence)...
              panzer.d.reverse().some(function (pkgDef) {
                var
                ...
                if (typeof def.prepNode == 'function' && typeof (substituteParent = def.prepNode.call(window, value, flags.returns[0].value)) != 'undefined') {
                  // scan this value as if it were the current node's value
                  flags.source = substituteParent;
                  // exit loop
                  return true;
                }
              });
      ...
      genCloneNodes = genData.spawn(
        function (name, value, parent, flags) {
          ...
          if (parent) {
            ...
            // add copy to the resulting array
            return this;
          }
        }
      ),
    ...
    function Tree(panzer, proxyInst, rawtree, pkgConfig) {
      ...
      tree.nodes = genNodes(rawtree, panzer);
    ...
    Tree.prototype = {
      ...
      go: function () {
        var
          ...
          traversals = 0,
          // flag when resuming traversal of a node
          ...
          nextNodeIndex = -1,
          // last targeted node index - use same node target if resuming
        ...
        // while looping...
        while (tree.loop) {
          // if there is a target and we haven't stopped (or we're resuming)...
          if (tree.target && !tree.stop) {
            ...
            if (lastTargetIndex != tree.target.index || !(~nextPhase | ~nextNodeIndex)) {
            ...
            } else if (~nextNodeIndex) { // if changing nodes...
              // if the current node was engaged...
              if (nodeEngaged) {
                // flag that this node is released
                nodeEngaged = 0;
                ...
                // fire node event and send new and old indice
            ...
            } else if (!nodeEngaged) { // or, when the current node has not been engaged...
                // flag that this node is engaged
                nodeEngaged = 1;
                // fire engage event and engage node
                tree.fire('engage');
            } else if (!curNode.ctx && (nextPhase == 1 || nextPhase == 2)) { // or, when out of context and traversing in or out...
              // if traversing out...
              if (nextPhase == 2) {
                // clear target phase
                nextPhase = -1;
            ...
            } else { // otherwise, when already scoped to traverse...
              ...
              if (resuming) {
                ...
                // clear targets when traversing "on" a node
                if (!nextPhase) {
                  tree.target = 0;
                  tank.targetIndex = -1;
                }
      ...
      if (arguments.length) {
        ...
        if (typeof name === 'string' && /\w/.test(name)) {
          ...
          if (!panzer.i.hasOwnProperty(name)) {
            ...
            pkgDef.init = pkgDef.tagKey = pkgDef.badKey = pkgDef.onBegin = pkgDef.onEnd = pkgDef.onTraverse = pkgDef.onTraversed = pkgDef.onTraversing = pkgDef.onEngage = pkgDef.onRelease = pkgDef.onScope = pkgDef.onNode = pkgDef.prepTree = pkgDef.prepNode = 0;
  ...
  // initialize and expose Panzer, based on the environment
  if (inCJS) {
